...
def _partidos(n: int) -> list:
    return [
        {"home_team": "A", "away_team": "B", "home_goals": 1, "away_goals": 0, "fecha": "2024-01-01"} for _ in range(n)
    ]
...
class TestHistoricoLargo(unittest.TestCase):
    def test_prefiere_ligamx_si_tiene_mas(self):
        with (
            mock.patch.object(fd.ligamx_api, "resultados_historicos", return_value=_partidos(500)),
    ...
    def test_cae_a_espn_si_ligamx_falla(self):
        with (
            mock.patch.object(fd.ligamx_api, "resultados_historicos", side_effect=RuntimeError("API caída")),
    ...
    def test_cae_a_cache_si_todo_falla(self):
        with (
            mock.patch.object(fd.ligamx_api, "resultados_historicos", return_value=[]),
