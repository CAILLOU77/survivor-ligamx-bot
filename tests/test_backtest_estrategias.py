...
def _torneo(d0: date, n_semanas: int) -> list:
    """
    Un torneo sintético: cada semana 'Fuerte' gana 3-0 de local y 'Local2' gana
    2-1. Los visitantes fuertes NO existen (para que el modelo tenga favoritos
    locales claros). Suficientes equipos para no agotar el pool.
    """
    out = []
...
def _dos_torneos() -> list:
    """Dos torneos separados por un hueco > 28 días (Apertura/Clausura)."""
    t1 = _torneo(date(2025, 1, 6), 12)
    t2 = _torneo(date(2025, 8, 4), 12)  # ~30 semanas después: hueco grande
    return t1 + t2
...
class TestDivisionTorneos(unittest.TestCase):
    def test_detecta_dos_torneos(self):
        r = be.simular_estrategia(_dos_torneos(), estrategia=be.estrategia_ingenua,
                                  min_train=6)
...
class TestSimulacion(unittest.TestCase):
    def test_estructura_agregados(self):
        r = be.simular_estrategia(_dos_torneos(), estrategia=be.estrategia_real,
                                  min_train=6)
...
class TestComparar(unittest.TestCase):
    def test_comparar_devuelve_ambas(self):
        r = be.comparar_estrategias(_dos_torneos(), min_train=6)
